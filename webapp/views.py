...
def home():
    ...
    post_id = db.session.query(Articles.id).order_by(
        Articles.date.desc()).limit(5).all()
    image = db.session.query(Articles.file_path).order_by(
        Articles.date.desc()).limit(5).all()
    
    
    return render_template("index.html", user=current_user, content=post,  post_title=post_title, post_date=post_date, post_author=post_author, post_id=post_id, image=image,  id="!!")
